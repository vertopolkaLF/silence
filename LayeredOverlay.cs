...
public sealed class LayeredOverlay : IDisposable
{
    ...
    private bool _isAnimationTickInProgress;
    ...
    public LayeredOverlay()
    {
        _dispatcherQueue = DispatcherQueue.GetForCurrentThread()
            ?? throw new InvalidOperationException("LayeredOverlay must be created on the UI thread.");
        _instance = this;
        RegisterWindowClass();
        CreateOverlayWindow();

        // Setup fade animation timer
        _fadeTimer = _dispatcherQueue.CreateTimer();
        _fadeTimer.Interval = TimeSpan.FromMilliseconds(FadeAnimationIntervalMs);
        _fadeTimer.IsRepeating = true;
        _fadeTimer.Tick += (_, _) => OnFadeTimerTick();
    }
    ...
    {
        RunOnUiThread(() => UpdateMuteStateCore(isMuted));
    }

    private void UpdateMuteStateCore(bool isMuted)
    {
    ...
    {
        RunOnUiThread(ApplySettingsCore);
    }
    ...
    {
        RunOnUiThread(() => SetButtonModeCore(enabled));
    }

    private void SetButtonModeCore(bool enabled)
    {
        if (_isDisposed) return;
    ...
    private void UpdateLayeredWindowBitmap(Bitmap bitmap, bool sizeChanged)
    {
        ...
        try
        {
            ...
            if (!UpdateLayeredWindow(_hwnd, screenDc, ref windowPoint, ref size, memDc, ref sourcePoint, 0, ref blend, ULW_ALPHA))
            {
                LogWin32Failure($"UpdateLayeredWindow failed while rendering overlay. Error={Marshal.GetLastWin32Error()}");
            }
        }
        finally
        {
    ...
    private void StartFadeTimerCore()
    {
    ...
    {
        if (_isDisposed || _isAnimationTickInProgress)
        {
            return;
        }

        _isAnimationTickInProgress = true;
        try
        {
        ...
        {
            _isAnimationTickInProgress = false;
        }
    }

    public void MoveToPosition(double percentX, double percentY, string screenId)
    {
        RunOnUiThread(() => MoveToPositionCore(percentX, percentY, screenId));
    }

    private void MoveToPositionCore(double percentX, double percentY, string screenId)
    {
        if (_isDisposed) return;
    ...
    {
        RunOnUiThread(StartPositioningCore);
    }

    private void StartPositioningCore()
    {
        if (_isDisposed) return;
    ...
    {
        RunOnUiThread(StopPositioningCore);
    }

    private void StopPositioningCore()
    {
        if (_isDisposed) return;
    ...
    private void SetClickThrough(bool clickThrough)
    {
        ...
        var previousExStyle = SetWindowLong(_hwnd, GWL_EXSTYLE, newExStyle);
        var setStyleError = Marshal.GetLastWin32Error();
        if (previousExStyle == 0 && setStyleError != 0)
        {
            LogWin32Failure($"SetWindowLong(GWL_EXSTYLE) failed while updating click-through mode. Error={setStyleError}");
        }

        const uint frameFlags = SWP_FRAMECHANGED | SWP_NOACTIVATE | SWP_NOMOVE | SWP_NOSIZE | SWP_NOOWNERZORDER;
        if (!SetWindowPos(_hwnd, HWND_TOPMOST, 0, 0, 0, 0, frameFlags))
        {
            LogWin32Failure($"SetWindowPos(SWP_FRAMECHANGED) failed while updating click-through mode. Error={Marshal.GetLastWin32Error()}");
        }

        EnsureTopMost("interaction-mode");
    }
    ...
    private void OnDisplayChangedCore()
    {
        ...
        if (settings != null)
        {
            MoveToPositionCore(settings.OverlayPositionX, settings.OverlayPositionY, settings.OverlayScreenId);
        }

        EnsureTopMost("display-change");
    }

    public void ProcessDrag()
    {
        RunOnUiThread(ProcessDragCore);
    }

    private void ProcessDragCore()
    {
        if (_isDisposed) return;
    ...
    public void Dispose()
    {
        ...
        {
            _dispatcherQueue.TryEnqueue(DisposeCore);
        }
    ...
    private void DisposeCore()
    {
        ...
        if (_hwnd != IntPtr.Zero)
        {
            ...
            {
                LogWin32Failure($"DestroyWindow failed for overlay. Error={Marshal.GetLastWin32Error()}");
            }
            _hwnd = IntPtr.Zero;
        }
    ...
    private void RunOnUiThread(Action action)
    {
        ...
        {
            System.Diagnostics.Debug.WriteLine("LayeredOverlay: failed to marshal action to UI thread.");
        }
    }
    ...
    private void EnsureTopMost(string reason)
    {
        ...
        const uint flags = SWP_NOACTIVATE | SWP_NOMOVE | SWP_NOSIZE | SWP_NOOWNERZORDER;
        if (!SetWindowPos(_hwnd, HWND_TOPMOST, 0, 0, 0, 0, flags))
        {
            LogWin32Failure($"SetWindowPos(HWND_TOPMOST) failed during {reason}. Error={Marshal.GetLastWin32Error()}");
        }
    }

    private static void LogWin32Failure(string message)
    {
        System.Diagnostics.Debug.WriteLine($"LayeredOverlay: {message}");
    }
